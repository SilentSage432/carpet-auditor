...
export function DeptSyncBadge({ size = "md", className = "" }: Props) {
  ...
  return (
    <span
      ...
      aria-hidden
    >
      <span className="absolute inset-0 rounded-xl bg-gradient-to-br from-emerald-950 via-slate-950 to-amber-950 ring-1 ring-emerald-500/40" />
      <span className="absolute inset-[2px] rounded-[10px] bg-slate-950/90" />
      {/* Boxes stacked */}
      <svg
        viewBox="0 0 24 24"
        className="relative z-[1] h-[55%] w-[55%] -translate-x-[10%] -translate-y-[8%] text-emerald-400"
        fill="currentColor"
      >
      ...
      <svg
        viewBox="0 0 24 24"
        className="absolute bottom-1 right-1 z-[2] h-[42%] w-[42%] text-amber-400 drop-shadow-[0_0_6px_rgba(251,191,36,0.45)]"
        fill="currentColor"
      >
