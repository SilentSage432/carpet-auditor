/**
 * DeptSync floating mark — Lucide layers (cyan) + barcode (gold).
 * Presentation only. Boot/PWA surfaces use `branded` so the mark never
 * inherits a theme accent (legacy emerald shield flash).
 */

type Props = {
  size?: "sm" | "md" | "lg";
  className?: string;
  /** Pin midnight cyan/gold so splash/install chrome matches PWA icons. */
  branded?: boolean;
};

const BRAND_CYAN = "#7dd3fc";
const BRAND_GOLD = "#fbbf24";
const BRAND_GLOW = "rgba(125, 211, 252, 0.55)";
  ...
  return (
    ...
    >
      <span
        ...
        style={{
          background: branded
            ? `radial-gradient(circle at 50% 50%, ${BRAND_GLOW} 0%, transparent 68%)`
            : "radial-gradient(circle at 50% 50%, var(--glow-accent) 0%, transparent 68%)",
        }}
      />
      <svg
        viewBox="0 0 24 24"
        className={`relative z-[1] h-[62%] w-[62%] -translate-x-[8%] -translate-y-[6%] ${
          branded ? "" : "text-accent drop-shadow-[0_0_10px_var(--glow-accent)]"
        }`}
        fill={branded ? BRAND_CYAN : "currentColor"}
        style={
          branded
            ? { filter: `drop-shadow(0 0 10px ${BRAND_GLOW})` }
            : undefined
        }
      >
      ...
      <svg
        viewBox="0 0 24 24"
        className={`absolute bottom-0.5 right-0.5 z-[2] h-[44%] w-[44%] ${
          branded ? "" : "text-warning drop-shadow-[0_0_8px_var(--glow-accent)]"
        }`}
        fill={branded ? BRAND_GOLD : "currentColor"}
        style={
          branded
            ? { filter: `drop-shadow(0 0 8px ${BRAND_GLOW})` }
            : undefined
        }
      >
