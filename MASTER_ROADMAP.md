...
- [ ] **UX-005G.1 Samsung field acceptance — Add Team Member focused workspace coverage** (engineering complete, **not** field accepted). Roster → Add Team Member was classified *ambiguous* by UX-005G archaeology and deliberately left unchanged; real Samsung evidence resolved it to a focused workspace, so it now claims the **existing** occupancy mechanism. Two lines: one import, one `useFocusedWorkspace()` call inside the conditionally-rendered sheet. **No new architecture.** The sixteen remaining A-class overlays stay evidence-gated
...
- [ ] **ROSTER-LIFE-001 Samsung field acceptance — removal integrity** (code repair complete, **not** field accepted). Removed members no longer reappear from the roster TTL cache, and the remove-confirm dialog suppresses BottomNav and clears the device safe area
- [ ] **ROSTER-EDIT-001 Samsung field acceptance — edit member workforce details** (code repair complete, **not** field accepted). `PATCH /api/roster/members/[id]` corrects **name, phone, and job title** on the existing row by immutable id, so a typo no longer means delete-and-recreate; `SpecialistEditSheet` gained an `Edit details` affordance. Authority-sensitive fields are refused outright and authorization mirrors the grant route. **No migration** — every column already existed
- [ ] **DECISION REQUIRED — may a member's home department be changed, and what happens to their access?** The only field left out of ROSTER-EDIT-001, because `accessible_departments` stores the home department **in the same flat array as granted extras with nothing distinguishing them**. A move can only either leave the old home as standing access nobody granted, or erase genuine cross-department grants — `AssociateRosterPanel` already does the former, which is a precedent but not a decided contract. **Recommendation: give the home department its own concept and derive access from it; do not add a third column** unless future evidence requires it. **PRODUCT LAW (recorded at ROSTER-EDIT-001 review):** *Changing a member's home department is a workforce reassignment and must not silently preserve or revoke cross-department authority. The user's intent for the old department's access must be explicitly resolved.* The eventual interaction — move Plumbing → Electrical, then explicitly **Keep Plumbing as additional access** or **Remove Plumbing access** — is **not to be built now** (ROSTER-EDIT-001)
