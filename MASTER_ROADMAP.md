...
- [ ] **DECISION REQUIRED — roster member correction (`PATCH /api/roster/members/[id]`).** There is **no** edit-member feature: `SpecialistEditSheet` renders identity read-only, and both existing writers are unsafe for rename (`saveSpecialist` upserts on `(store_number, name)` and swallows the failure as `offline: true`; `persistSpecialistFields` hijacks session identity offline). Correcting a typo currently means delete-and-recreate. **No migration needed** — every column exists. Needs an authority decision on who may change name / phone / job title / department (ROSTER-LIFE-001)
