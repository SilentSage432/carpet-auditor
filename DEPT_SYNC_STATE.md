...
| Floor attention summary (SI-001C) | `location-attention-summary.ts` + `FloorAttentionSummary` + FloorTab | **COMMITTED AND PUSHED** (`21e1a72`) — **DEPLOYMENT SHA VERIFICATION PENDING**; pure MEDIUM/HIGH tier counts; independent Floor SI fetch; Master all gated; staging/shift do not refetch SI; verify/barrier success notifies locations-changed; not LIVE until Vercel SHA confirmed |
