...
| Floor → Map investigation (UX-004) | `map-attention-investigation.ts` + Floor CTA URL intent + MapTab strip / marker emphasize | **IMPLEMENTED** — navigation intent only; Map resolves current SI; geography preserved; Standard on arrival; Show all clears; no ranking/REC/LAB/schema/API/SI semantics; UX-004A real-device usable w/ follow-ups |
| Quiet Show all clear (UX-004B) | `exitMapAttentionInvestigation` + `replaceState(history.state)` then `router.replace` | **IMPLEMENTED** — clear independent of SI result; history.state preserved; elevated preserved; device re-smoke required; UX004A-02/03 deferred |
