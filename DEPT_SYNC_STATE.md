...
| Verification authority (UX-002 · THESIS-001A) | Floor strip → `SupervisorAuditSummaryModal` → `review_action`; week stamp via `stampDepartmentWeekVerified` only after true verify_all | **IMPLEMENTED + AUTHORITY-HARDENED** — false Shift Analytics empty-ID CTA + `verifyAllCompletedBays` removed; sole bay-review owner = modal; **legacy ungated batch verify REMOVED (not re-gated) by THESIS-001A** — `POST /api/rotations/verify` now requires supervisor/admin for the whole method and rejects any request without an explicit `review_action`; `verification.ts` can no longer write `VERIFIED_COMPLETE` / location `COMPLETED` at all; verify + send-back guard the stored review state (raw column, so pre-migration rows still verify); no schema change |
