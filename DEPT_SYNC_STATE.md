...
**Appliance specialty product evolution (directional, not contractual):** [`docs/product/APPLIANCE_EVOLUTION_ROADMAP.md`](docs/product/APPLIANCE_EVOLUTION_ROADMAP.md). Dependency law: reliable controls → audit lifecycle → richer observations → recurrence → longitudinal evidence → deterministic intelligence. **APP-AUD-002A/002B** live (lifecycle + freeze). **APP-OBS-001** fulfillment disposition production migration LIVE (real-hardware validation pending). **APP-CAT-001A** multi-identifier resolution production migration LIVE (ESL / real-hardware validation pending). **APP-CAT-001B** local promotion deferred. Next after CAT field acceptance: **APP-ROT-001**. Parking lot items stay out of schedule until prerequisites clear.
