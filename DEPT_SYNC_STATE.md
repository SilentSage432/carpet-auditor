...
| Floor attention summary (SI-001C) | `location-attention-summary.ts` + `FloorAttentionSummary` + FloorTab | **IMPLEMENTED LOCALLY — AWAITING REVIEW**; pure MEDIUM/HIGH tier counts; independent Floor SI fetch (bounded duplicate GET with Map); Master all gated; staging/shift do not refetch SI; no score/rank/top-list; View on Map CTA |
