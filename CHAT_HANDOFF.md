...
- Persist owner: `lib/store-ops/rotations.ts` (`upsertWeeklyRotations`). Payload includes `store_id` (UUID from `20260809_multi_store.sql`) and `store_number` (JWT RLS from `20260812_jwt_rls_policies.sql`). If PostgREST reports a missing store column (PGRST204 / schema cache), that column is dropped and the upsert retries. A NOT NULL after a cache miss means Postgres has the column but the API cache is stale — reload schema (`NOTIFY pgrst, 'reload schema'`) rather than guessing the column set
- Upsert `onConflict` is **`location_id,assigned_week`** — the table unique from `20260809_store_operations_rbac.sql`, re-asserted by **`20260817_weekly_rotations_location_week_unique.sql`**. There is no `week_number` column; do not conflict on `store_number,department_id,week_number`. If the live unique is missing, persist merges by location+week instead of throwing a constraint mismatch
