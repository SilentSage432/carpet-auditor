...
## Latest (2026-09-05)
- **FS-002 foundation LIVE:** `operational_contexts` + `operational_context_department_relevance` — Master-declared seasons/events (Gregorian ranges), department relevance (UNSET vs NONE), provenance. No company-event seed, recurrence, location priority, or seasonal intelligence. APIs: `GET /api/operational-contexts`, Master CRUD under `/api/admin/operational-contexts*`. Settings: `OperationalContextCard`. Floor chip deferred (**FS-002B**). Next: FS-002B surface or FS-003 location relevance.
