...
## Latest (2026-09-06)
- **TEMPORAL COPY CLARIFICATION IMPLEMENTED LOCALLY — AWAITING COMMIT REVIEW:** Floor week chrome now says **Staging week** (`composeFloorWeekProgressWithStagingWeek`); Map chrome says **Calendar week** (`formatMapCalendarWeekChrome`). Clocks unchanged (`sundayStagingWeekLabel` / `isoWeekLabel`); Sunday W37 vs W36 divergence intentional; fiscal week separate; Map overlay `assignedWeek`/staging-week semantics deferred (UX-005). No cron/rotation/API/schema. Not LIVE.
