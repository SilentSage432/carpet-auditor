...
## Latest (2026-09-06)
- **UX-004 FLOOR → MAP INVESTIGATION CONTEXT IMPLEMENTED:** Floor Current Attention → Map preserves why the DS navigated (`investigate=current-attention&dept=`). Floor passes intent only; Map resolves current SI itself (no stale Floor evidence/location list). Geography preserved with marker emphasis; Standard on investigation arrival; Show all clears; MEDIUM/HIGH elevated set unchanged; no ranking/recommendation/Map redesign/SI semantic change/LAB/REC/schema/API. Feature freeze remains outside UI program. Commit/push ≠ real-device navigation smoke. Not production-mobile-validated.
