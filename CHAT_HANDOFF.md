...
- Master: store number, Sunday auto-stage time + auto-run (`SundayScheduleCard`), Store Topology & Bay Setup (`#bulk-generate`), Taxonomies (`#taxonomies`), Force Rotation (`#weekly-rotation`)
- Supervisor + Master: weekly targets matrix + Store Topology (Add Single Bay / Bulk Generator / Delete Selected). Floor Pad lives in Floor `ShiftAnalyticsDrawer` (`TacticalVoiceFloorPad`); Settings `#manager-notes` redirects to `/dashboard#floor-pad`
