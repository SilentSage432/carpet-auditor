...
## Latest (2026-09-06)
- **SI-001C FLOOR ATTENTION SUMMARY COMMITTED AND PUSHED — DEPLOYMENT SHA VERIFICATION PENDING:** Commit `21e1a72` on `main`. Floor compact **Current attention** strip after readiness/fiscal context. Pure tier counts over same `GET /api/store-intelligence/attention` as Map (`location-attention-summary.ts`). MEDIUM/HIGH foreground; NONE/LOW quiet; no department score/rank/top-list/filter/heatmap. Floor owns independent fetch (keep-alive → bounded duplicate GET with Map accepted). Master `all` → select department. Degraded keeps counts + “Some evidence unavailable”; failure → unavailable (not quiet zeros). Device-local `As of` via `generated_at`. Optional **View on Map** → `/admin/store-map`. Staging/Sunday + shift-status do **not** independently refetch SI; successful verify-batch/barrier mutations notify `STORE_OPS_LOCATIONS_CHANGED` so attention consumers refresh. No schema/API/SI-001/A/B semantic changes. Production smoke (unauthenticated) OK; **exact Vercel production SHA not verified** (CLI token expired / GH deployments API forbidden) — do not mark LIVE.
- **SI-001B MAP ATTENTION SURFACE IMPLEMENTED:** Map shows MEDIUM/HIGH compact Focus+Med/High markers; NONE/LOW quiet; Walk sheet Current attention detail (confidence/actionability/reasons/derived/`As of` **device-local** display of response `generated_at`); seasonal copy uses SI `effect` (CONTEXT vs MODIFY); Master `all` → select department; failure-independent; no sort/filter/heatmap/ranking/rotation coupling. Awaiting production deploy confirmation (not yet LIVE).
