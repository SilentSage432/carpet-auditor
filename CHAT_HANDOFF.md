...
## Latest (2026-09-07)
- **APP-AUD-002B PHYSICAL EVIDENCE FREEZE HARDENING — IMPLEMENTED — AWAITING COMMIT / MIGRATION APPLY / FIELD VALIDATION:** Option B — explicit `audit_session_id` membership (omit = unbound; no server ACTIVE auto-bind); ad-hoc + Floor/SIMS stay unbound; CLOSED freezes WHAT/WHERE/WHEN/WHICH via API + `20260907_appliance_scans_closed_evidence_freeze.sql`; soft classification fields remain mutable; late in-window offline bind (001A) preserved; catalog snapshot deferred. Next after 002 closes: **APP-OBS-001**.
