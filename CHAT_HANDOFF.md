...
## Latest (2026-09-06)
- **SI-001C FLOOR ATTENTION SUMMARY IMPLEMENTED LOCALLY — AWAITING REVIEW:** Floor compact **Current attention** strip after readiness/fiscal context. Pure tier counts over same `GET /api/store-intelligence/attention` as Map (`location-attention-summary.ts`). MEDIUM/HIGH foreground; NONE/LOW quiet; no department score/rank/top-list/filter/heatmap. Floor owns independent fetch (keep-alive → bounded duplicate GET with Map accepted). Master `all` → select department. Degraded keeps counts + “Some evidence unavailable”; failure → unavailable (not quiet zeros). Device-local `As of` via `generated_at`. Optional **View on Map** → `/admin/store-map`. Staging/Sunday + shift-status do **not** independently refetch SI; `STORE_OPS_LOCATIONS_CHANGED` + verify/barrier mutation notify do. No schema/API/SI-001/A/B semantic changes. Not LIVE / not deployed.
