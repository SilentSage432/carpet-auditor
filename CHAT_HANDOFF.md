...
## Latest (2026-09-05)
- **FS-001A minimum LIVE:** Derived fiscal coverage (`computeFiscalCoverage`) + Master-only `GET /api/admin/fiscal-calendar/coverage` + More→Settings `FiscalCoverageCard`. No discovery, no promote API, no persistence, no cron/push. FY2027 still not loaded — coverage signal only. **FS-002** seasons/events next after validation.
