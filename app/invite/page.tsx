...
/**
 * Supervisor invite onboarding — /invite?token=
 * Steps: temp PIN → Create New PIN → Add to Home Screen → Face ID / Fingerprint → done.
 */
...
function InviteBody() {
  ...
  const testMode =
    search.get("test") === "1" ||
    search.get("test") === "true" ||
    process.env.NODE_ENV === "development";
  const dryRun = search.get("test") === "1" || search.get("test") === "true";
  ...
  async function handleBiometric() {
    ...
    try {
      if (dryRun) {
        // Still fire WebAuthn when possible so Super Admin can rehearse UX;
        // failures are non-blocking in harness mode.
        try {
  ...
  return (
    <Shell>
      ...
      {step === "new-pin" ? (
        ...
        >
          <h2 className="text-base font-bold text-slate-50">Create New PIN</h2>
          <p className="text-sm text-slate-400">
            Choose a permanent 4–8 digit PIN before opening your dashboard.
          </p>
          <NumberField
            label="New PIN"
            mode="digits"
          ...
          <NumberField
            ...
            mode="digits"
            value={confirmPin}
            onChange={(v) => setConfirmPin(v.replace(/\D/g, "").slice(0, 8))}
